...
namespace ab
{
	namespace util
	{
		...
		{
			spdlog::info("Creating new PROJ object and context set");
			PJ_CONTEXT* projCtx = proj_context_create();
#ifndef NDEBUG
			if (projCtx == nullptr)
			{
				spdlog::error("PROJ context is null");
			}
			else
			{
				spdlog::info("PROJ context created");
			}
#endif
			// proj_context_set_enable_network(projCtx, true);
			// #ifdef PROJ_DATA_PATH
			// const char* projDataPaths[2];
			// std::cout << "Creating PROJ context with data path " << PROJ_DATA_PATH << "\n";
			// projDataPaths[0] = PROJ_DATA_PATH;
			// projDataPaths[1] = "/usr/share/proj/";
			// proj_context_set_search_paths(projCtx, 2, projDataPaths);
			// #endif
			PJ* reproj = proj_create_crs_to_crs(projCtx, sourceCRS, destCRS, nullptr);
			if (reproj == nullptr)
			{
				const auto errStr = std::string(proj_context_errno_string(projCtx, proj_errno(reproj)));
				spdlog::error("PROJ cs2cs object null");
				spdlog::error("PROJ error: " + errStr);
				proj_context_destroy(projCtx);
				throw std::system_error(std::make_error_code(std::errc::state_not_recoverable),
				                        "PROJ object cannot be null: " + errStr);
			}
		                                               ...
		                                               const GEOSCoordSequence* in)
		{
			...
			for (unsigned int i = 0; i < nCoords; i++)
			{
				...
				// Reproject coords
				PJ_COORD c = proj_trans(reprojector, PJ_FWD, proj_coord(cy, cx, cz, 0));
		...
		static GEOSCoordSequence* reprojectCoordinates_r(PJ* reprojector,
		                                                 ...
		                                                 const bool sourceEPSG4326 = false,
		                                                 const bool targetEPSG4326 = false)
		{
			unsigned int nCoords, nDims;
			...
			for (unsigned int i = 0; i < nCoords; i++)
			{
				...
				// Reproject coords
				PJ_COORD c = sourceEPSG4326
					             ? proj_trans(reprojector, PJ_FWD, proj_coord(cy, cx, cz, 0))
					             : proj_trans(reprojector, PJ_FWD, proj_coord(cx, cy, cz, 0));

				// Write reprojected coords to output coord seq
				if (targetEPSG4326)
				{
					GEOSCoordSeq_setOrdinate_r(geosCtx, outCoordSeq, i, 0, c.xyz.y);
					GEOSCoordSeq_setOrdinate_r(geosCtx, outCoordSeq, i, 1, c.xyz.x);
				}
				else
				{
		                                    ...
		                                    const char* destCRS = "EPSG:3395")
		{
...
#endif
				proj = proj_create_crs_to_crs(projCtx, sourceCRS, destCRS, nullptr);
			}
			else
			{
				proj = reproj;
			...
			const PJ_COORD out =
				proj_trans(proj, PJ_FWD, proj_coord(coordX, coordY, coordZ, coordT));
			if (reproj == nullptr)
			{
				proj_destroy(reproj);
				proj_context_destroy(projCtx);
		...
		static PJ_COORD reprojectCoordinate_r(PJ* reproj, const double coordX, const double coordY,
		                                      const double coordZ = 0, const double coordT = 0)
		{
			const PJ_COORD out =
